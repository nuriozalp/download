...
for DEVICE in /dev/tty*; do
    ...
    if udevadm info --query=property --name=$DEVICE | grep -q "ID_VENDOR_ID=$VENDOR_ID"; then
        ...
        if [ ! -z "$PRODUCT_ID" ]; then
            echo "RFID device found: $DEVICE with PRODUCT_ID=$PRODUCT_ID"
            
            # Udev kuralını tanımla
            RULE="SUBSYSTEM==\"tty\", ATTRS{idVendor}==\"$VENDOR_ID\", ATTRS{idProduct}==\"$PRODUCT_ID\", TEST==\"power/control\", SYMLINK+=\"msfrfid\""
            
            # Udev kuralını dosyaya yaz
            echo "$RULE" > /etc/udev/rules.d/msf_rfid_card_setting.rules
            
            # Udev kurallarını yeniden yükle
            echo "Reloading udev rules..."
            udevadm control --reload-rules && udevadm trigger
            
            echo "Udev rule for RFID device $DEVICE with PRODUCT_ID=$PRODUCT_ID is created."
            exit 0 # İlk uygun cihaz için kural oluşturulduktan sonra çık
        fi
    fi
done

echo "No RFID device with vendor ID $VENDOR_ID found. Exiting."
exit 1